...
function App(): JSX.Element {
  ...
  const mapData = (data: namesType[]) => {
    return data.map((name: namesType, index) => (
      <button
        key={name.id}
        className={name.sex === "f" ? "girl" : "boy"}
        onClick={() => handleFavourite(name.name, name.id, name.sex)}
      >
        {" "}
        {name.name}{" "}
      </button>
  ...
  const filterData = (data: namesType[]) => {
    return data.filter(
      (profile) =>
        profile.name.slice(0, typedMessage.length) ===
        typedMessage.charAt(0).toUpperCase() + typedMessage.slice(1)
    );
  };
  ...
  const handleFavourite = (name: string, id: number, sex: string) => {
    ...
    if (alreadyHere) {
      ...
      setNames((prev) =>
        [...prev, { id: id, name: name, sex: sex }].sort((a, b) =>
  ...
  return (
    <section>
      ...
      <button className="boy" onClick={handleJustBoys}>
        Boys
      </button>
      <button className="girl" onClick={handleJustGirls}>
        Girls
      </button>
